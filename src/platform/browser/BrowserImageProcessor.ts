...
class BrowserImageInstance implements IImageInstance {
  ...
  constructor(buffer: Buffer) {
    ...
    this.canvas = useOffscreen ? new OffscreenCanvas(1, 1) : document.createElement('canvas');
  ...
  static createBlank(options: CreateImageOptions): BrowserImageInstance {
    ...
    instance.loadPromise = (async () => {
      ...
      instance.ctx = instance.canvas.getContext('2d', { willReadFrequently: true }) as
        | CanvasRenderingContext2D
        | OffscreenCanvasRenderingContext2D;
  ...
  resize(width: number, height: number, options?: ResizeOptions): IImageInstance {
    ...
    this.loadPromise = (async () => {
      ...
      const newCtx = newCanvas.getContext('2d', { willReadFrequently: true }) as
        | CanvasRenderingContext2D
        | OffscreenCanvasRenderingContext2D;
      ...
      // Draw resized image
      newCtx.drawImage(oldCanvas as CanvasImageSource, 0, 0, targetWidth, targetHeight);
