...
import { ValidationError, ImageError } from '../../errors';
...
export class BrowserFileSystem implements IFileSystem {
  async readFile(source: string | File | Buffer): Promise<Buffer> {
    ...
    throw new ValidationError('Invalid source type for readFile in browser', 'source');
  }
  ...
  private async readFileFromURL(url: string): Promise<Buffer> {
    try {
      ...
      if (!response.ok) {
        throw new ImageError(`HTTP error! status: ${response.status}`, url);
      }
