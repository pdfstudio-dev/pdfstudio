...
export class ImageParser {
  ...
  static async load(source: string | File | Buffer): Promise<ImageInfo> {
    ...
    switch (format) {
      ...
      default:
        throw new ImageError(`Unsupported image format: ${format}`);
    }
  }
  ...
  private static detectFormat(buffer: Buffer): ImageFormat {
    ...
    throw new ImageError('Unknown image format. Only JPEG and PNG are supported.');
  }
  ...
  private static parseJPEG(buffer: Buffer): ImageInfo {
    ...
    if (width === 0 || height === 0) {
      throw new ImageError('Could not determine JPEG dimensions');
    }
  ...
  private static parsePNG(buffer: Buffer): ImageInfo {
    ...
    let transparency: Buffer | undefined = undefined;
    ...
    if (width === 0 || height === 0) {
      throw new ImageError('Could not determine PNG dimensions');
    }

    if (idatChunks.length === 0) {
      throw new ImageError('No IDAT chunks found in PNG');
    }
    ...
    for (let y = 0; y < height; y++) {
      ...
      const scanline = Buffer.from(
        decompressed.slice(scanlineStart + 1, scanlineStart + 1 + scanlineLength)
      );

      const prevScanline =
        y > 0
          ? Buffer.from(unfilteredData.slice((y - 1) * scanlineLength, y * scanlineLength))
          : null;
  ...
  private static unfilterScanline(
    ...
    for (let i = 0; i < scanline.length; i++) {
      const x = scanline[i];
      const a = i >= bytesPerPixel ? result[i - bytesPerPixel] : 0; // Left pixel
      const b = prevScanline ? prevScanline[i] : 0; // Above pixel
      const c = prevScanline && i >= bytesPerPixel ? prevScanline[i - bytesPerPixel] : 0; // Upper left
