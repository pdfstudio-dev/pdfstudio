...
import { ImageError, ValidationError } from '../errors';
...
export class QRCodeGenerator {
  ...
  static async generate(options: QRCodeOptions): Promise<Buffer> {
    ...
    try {
      ...
      if (isBrowser) {
        ...
        const bufferOptions: QRCode.QRCodeToBufferOptions = { ...qrOptions, type: 'png' };
        qrBuffer = await (QRCode.toBuffer(qrText, bufferOptions) as unknown as Promise<Buffer>);
      }
    } catch (error) {
  ...
  private static convertDataToText(data: string | QRCodeData): string {
    ...
    throw new ValidationError('No valid QR code data provided', 'data');
  }
  ...
  private static async addLogoToQR(
    ...
    try {
      ...
      const logoSizePercent = Math.min((logoOptions.width || qrWidth * 0.2) / qrWidth, 0.3);
      const logoSize = Math.floor(qrWidth * logoSizePercent);

      // Resize logo
      const logoProcessed = imageProcessor.load(logoBuffer).resize(logoSize, logoSize, {
        fit: 'contain',
      ...
      if (logoOptions.borderRadius && logoOptions.borderRadius > 0) {
        ...
        logoImageBuffer = await imageProcessor
          .load(logoImageBuffer)
          .composite([
            {
              input: maskBuffer,
              top: 0,
      ...
      const logoBackground = await imageProcessor
        .create({
          width: bgSize,
          height: bgSize,
          channels: 4,
      ...
      logger.error('Error adding logo to QR code, returning original QR code', 'QRCodeGenerator', {
