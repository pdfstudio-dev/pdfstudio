...
export class FontManager {
  ...
  async loadFont(customFont: CustomFont): Promise<FontInfo> {
    ...
    if (Buffer.isBuffer(customFont.source)) {
      ...
      typeof customFont.source === 'string' ||
      (typeof customFont.source === 'object' &&
        'name' in customFont.source &&
        'size' in customFont.source)
    ) {
      ...
      if (
        !fs.isBrowser() &&
        typeof customFont.source === 'string' &&
        !fs.existsSync(customFont.source)
      ) {
        throw new FontError(`Font file not found: ${customFont.source}`, customFont.name);
      }
      fontData = await fs.readFile(customFont.source);
    } else {
      throw new FontError('Invalid font source: must be a file path, URL, File object, or Buffer');
    }
...
CMapName currentdict /CMap defineresource pop
  ...
  generateSubsettedFont(fontInfo: FontInfo): { data: Buffer; isSubset: boolean } {
    ...
    try {
      ...
      logger.info(`Font subsetting complete for ${fontInfo.name}`, 'FontManager', {
