...
export function validateFiniteNumber(
  ...
  if (!allowZero && value === 0) {
    throw new ValidationError(`${paramName} cannot be zero`, paramName, value);
  }

  if (!allowNegative && value < 0) {
    throw new ValidationError(`${paramName} must be non-negative, got: ${value}`, paramName, value);
  }
}
 ...
 */
export function validateRectangle(x: number, y: number, width: number, height: number): void {
 ...
 */
export function validateNonEmptyArray<T>(array: T[], paramName: string): void {
  ...
  if (array.length === 0) {
    throw new ValidationError(`${paramName} cannot be empty`, paramName, array);
  }
}
 ...
 */
export function validatePageIndex(pageIndex: number, totalPages: number): void {
