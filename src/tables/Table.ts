...
export class Table {
  ...
  render(): void {
    ...
    if (!autoPageBreak) {
      if (
        this.shouldDrawBorder('top') ||
        this.shouldDrawBorder('bottom') ||
        this.shouldDrawBorder('left') ||
        this.shouldDrawBorder('right')
      ) {
        const totalHeight =
          (this.options.headerStyle?.height || this.options.rowHeight!) +
