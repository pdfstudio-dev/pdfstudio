...
export class PieChart extends BaseChart {
  ...
  constructor(
    writer: PDFWriter,
    options: PieChartOptions | DonutChartOptions,
    ...
    if (isDonut && 'outerRadius' in options) {
      const donutOpts = options as DonutChartOptions;
      validatePositiveNumber(donutOpts.outerRadius, 'outerRadius');
      validatePositiveNumber(donutOpts.innerRadius, 'innerRadius');
      if (donutOpts.innerRadius >= donutOpts.outerRadius) {
        throw new ChartDataError(
          `Inner radius (${donutOpts.innerRadius}) must be less than outer radius (${donutOpts.outerRadius})`,
          'DonutChart',
          'invalid radii'
        );
      }
    } else if ('radius' in options) {
      validatePositiveNumber((options as PieChartOptions).radius, 'radius');
    }
    ...
    if (isDonut && 'outerRadius' in options) {
      defaultLabelDistance =
        (options as DonutChartOptions).outerRadius + C.PIE_LABEL_DISTANCE_FROM_EDGE;
    } else if ('radius' in options) {
      defaultLabelDistance = (options as PieChartOptions).radius + C.PIE_LABEL_DISTANCE_FROM_EDGE;
    } else {
  ...
  /**
   * Get the effective outer radius for the chart
   */
  private getRadius(): number {
    return this.isDonut && 'outerRadius' in this.options
      ? (this.options as DonutChartOptions).outerRadius
  ...
  render(): void {
    ...
    if (title) {
      ...
      const titleY = y + this.getRadius() + C.TITLE_VERTICAL_OFFSET;
    ...
    coloredSlices.forEach((slice, index) => {
      this.drawSlice(x, y, currentAngle, slice.angle, slice.color, slice.label, slice.percentage);
  ...
  private drawSlice(
    ...
    const innerRadius =
      this.isDonut && 'innerRadius' in this.options
        ? (this.options as DonutChartOptions).innerRadius
        : 0;
  ...
  private drawArcRadians(
    ...
    const angleDiff = endAngleRad - startAngleRad;
  ...
  private drawLabel(
    ...
    const labelX =
      centerX + labelDistance * Math.cos(angleRad) - label.length * C.PIE_LABEL_CHAR_WIDTH;
    ...
    if (this.options.showPercentages) {
      ...
      this.writer.text(
        percentText,
        percentX,
        labelY - C.PIE_PERCENTAGE_VERTICAL_OFFSET,
        C.PIE_PERCENTAGE_FONT_SIZE
      );
    }
  }
  ...
  private drawLegend(items: LegendItem[], chartX: number, chartY: number): void {
    ...
    items.forEach((item, index) => {
      ...
      this.writer.rect(
        legendX,
        itemY - C.PIE_LEGEND_BOX_VERTICAL_ADJUST,
        C.PIE_LEGEND_BOX_WIDTH,
        C.PIE_LEGEND_BOX_HEIGHT
      );
      this.writer.fill();
  ...
  private drawBorder(chartX: number, chartY: number): void {
    ...
    if (this.options.legend?.show) {
      ...
      const legendVerticalSpace =
        numItems * (fontSize + itemSpacing) + C.PIE_LEGEND_VERTICAL_SPACE_MARGIN;
